...
> 2026-05-14 覆盖说明：本文件记录的是当前历史主题生成技能与视觉规则。产品目标已切换到 `Portal Agent`，视觉主题生成只是门户生成子能力，不能再把本文件当成完整产品定义。

### AI 工作范围：创建 → 迭代

旧的 Theme Agent 职责是帮用户完成**视觉创意设计**。当前产品目标已升级为 Portal Agent：先补齐客户信息，再生成门户初稿。这里记录的规则主要继续约束视觉主题生成部分。
...
> 注意：这里约束的是 `Portal Agent` 中的视觉主题子能力，不是完整门户生成职责。
