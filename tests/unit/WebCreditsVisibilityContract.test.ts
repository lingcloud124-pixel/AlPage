...
describe('web credits visibility contract', () => {
  test('landing and sidebar credits are hidden by default in markup until runtime config loads', () => {
    ...
    expect(html).toContain('id="landingCreditsChip" class="landing-credits-chip" type="button" style="display: none;"');
    expect(html).toContain('id="landingCreditsTooltipList"');
